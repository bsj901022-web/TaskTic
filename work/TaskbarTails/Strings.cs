...
public static class L
{
    ...
    static readonly Dictionary<string, (string ko, string en)> Table = new()
    {
        ...
        ["set_note"] = ("설정은 바로 적용되고 저장됩니다. 레벨 보상: Lv.3 민트 말풍선 · Lv.5 청록 이름+배지 · Lv.8 피치 · Lv.10 금색 이름 · Lv.15 반짝임 · Lv.20 왕관. 친구에게도 보여요.", "Settings apply immediately and are saved. Level rewards: Lv.3 mint bubble · Lv.5 teal name + badge · Lv.8 peach · Lv.10 gold name · Lv.15 sparkles · Lv.20 crown. Friends see them too."),
        ...
        ["set_edge_roam"] = ("화면 가장자리 타고 돌기 (좌우 벽 오르기·천장 걷기)", "Roam the screen edges (climb the sides, walk the ceiling)"),
        ["edge_climb"] = ("벽 타기 시작!", "Up the wall I go!"),
        ["edge_down"] = ("다시 바닥이다", "Back on the ground"),
        ["edge_letgo"] = ("어이쿠, 손이 미끄러졌다!", "Whoops, slipped!"),
        ["level_up"] = ("레벨 업! Lv.{0}!", "Level up! Lv.{0}!"),
        ["level_up_notice"] = ("Lv.{0} 달성! {1}", "Reached Lv.{0}! {1}"),
        ["friend_level_up"] = ("{0}가 Lv.{1}이 됐어! 축하해", "{0} reached Lv.{1}! Congrats"),
        ["xp_gain"] = ("경험치 +{0} · 오늘 돌봄 {1}/{2}", "XP +{0} · care today {1}/{2}"),
        ["xp_not_hungry"] = ("배가 별로 안 고파서 경험치는 없어요. 포만감 70 아래에서 먹이면 +5.", "Not hungry enough for XP. Feeding below 70 fullness gives +5."),
        ["xp_play_wait"] = ("놀이 경험치는 2분마다 한 번이에요. 이번엔 그냥 신나게!", "Play XP comes once every 2 minutes. This one was just for fun!"),
        ["xp_click_cap"] = ("오늘의 인사 경험치({0})는 다 받았어요. 인사는 언제든 환영!", "Today's greeting XP ({0}) is all collected. Greetings are always welcome!"),
        ["xp_care_cap"] = ("오늘 돌봄 경험치 {0}을 다 채웠어요. 함께 있는 시간(10분마다 +1)과 친구 활동으로 계속 올라가요.", "Today's care XP ({0}) is full. Time together (+1 per 10 min) and friends keep it growing."),
        ["xp_today"] = ("오늘 경험치 · 돌봄 {0}/{1} · 함께한 시간 {2}/{3} · 친구 {4}/{5}", "XP today · care {0}/{1} · time together {2}/{3} · friends {4}/{5}"),
        ["next_perk"] = ("다음 보상 Lv.{0}: {1}", "Next reward at Lv.{0}: {1}"),
        ["perks_done"] = ("모든 레벨 보상을 받았어요!", "All level rewards unlocked!"),
        ["perk_3"] = ("민트 말풍선", "mint bubble"),
        ["perk_5"] = ("청록 이름 + 레벨 배지(친구에게도 보임)", "teal name + level badge (friends see it too)"),
        ["perk_8"] = ("피치 말풍선", "peach bubble"),
        ["perk_10"] = ("금색 이름", "gold name"),
        ["perk_15"] = ("반짝임 효과", "sparkles"),
        ["perk_20"] = ("왕관 + 그라데이션 이름", "crown + gradient name"),
        ["perk_none"] = ("새 모습!", "A new look!"),
