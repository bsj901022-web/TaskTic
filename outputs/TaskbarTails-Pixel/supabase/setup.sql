-- Taskbar Tails v0.5 / Supabase setup (fresh install; existing v0.4 projects run upgrade-v05.sql instead)
-- Run the entire file once in the project's SQL Editor as postgres.
-- Only tt_* tables/functions and the tt_room_subscriptions policy are created.
-- No existing application tables are deleted or modified.
begin;
...
-- The sender identity is assigned by the server, never trusted from the app.
-- Motion snapshots and bubbles use private Realtime broadcasts, not per-frame DB writes.
...
select 'Taskbar Tails tables and private room functions are ready' as result;
