...
// PixelLab sprites only. The preview is large; desktop pets use one third of the v0.2 size times the size setting.
public sealed class PetVisual : FrameworkElement
{
    ...
    public bool Parachute, BallVisible, GoldName;
    ...
    public PetVisual() { RenderOptions.SetBitmapScalingMode(this, BitmapScalingMode.NearestNeighbor); }
    ...
    protected override void OnRender(DrawingContext d)
    {
        ...
        d.DrawEllipse(Shadow, null, new Point(80, 174), Math.Max(5, 25 * petScale - Jump / 12), FrontView ? 4 : 2 * Math.Max(1, SizeFactor * .75));
